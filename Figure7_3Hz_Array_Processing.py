...
# Load data
st = obspy.read("Array_3Hz_Sep10_Restart.mseed")
...
starttime = (2019,10,1)
stime=obspy.UTCDateTime("2019-253T21:34:50")
etime = stime + 180.
...
for tr in st: 
    ...
    tr.filter('bandpass',freqmin=3.11,freqmax=3.19)
    
    
# Execute array_processing
kwargs = dict(
    # slowness grid: X min, X max, Y min, Y max, Slow Step
    sll_x=-5.0, slm_x=5.0, sll_y=-5.0, slm_y=5.0, sl_s=0.03,
    # sliding window properties
    win_len=0.5, win_frac=0.1,
    # frequency properties
    frqlow=3.11, frqhigh=3.19, prewhiten=0,
    # restrict output
...
plt.subplot(411)
plt.plot(times,tr_plot.data*1000000000.,linewidth=1,c='k')
plt.ylabel('Ground\nVelocity (nm/s)', fontsize=18)
plt.tick_params(labelsize = 15) 
plt.text(-23,2.5,'(a)', fontsize=20)
plt.ylim(-3, 3)
plt.xlim(0, 180)
plt.xticks([])
...
plt.scatter(t,baz, c=rel_power, alpha=0.8,
               edgecolors='none', cmap=obspy_sequential)
plt.ylabel('Backazimuth ($^\circ$)', fontsize=18) 
plt.ylim(0, 360)  
plt.xlim(0, 180)
plt.xticks([])
plt.yticks([90,180,270])
plt.tick_params(labelsize = 15) 
plt.text(-23,360.0,'(c)', fontsize=20)
plt.text(-23,0,'(d)', fontsize=20)
